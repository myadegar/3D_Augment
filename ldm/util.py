...
def add_margin(pil_img, color, size):
    ...
    result = Image.new(pil_img.mode, size, color)
    result.paste(pil_img, ((size[0] - width) // 2, (size[1] - height) // 2))
    return result
...
def load_and_preprocess(interface, input_im, size):
    '''
    ...
    low_size = [size[0] - 64, size[1] - 160]
    image.thumbnail(low_size, Image.Resampling.LANCZOS)
    image = add_margin(image, (255, 255, 255), size=size)
    image = np.array(image)
